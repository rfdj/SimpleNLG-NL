...
public class NounPhraseHelper extends AbstractNounPhraseHelper
{
	...
	protected NLGElement createPronoun(PhraseElement phrase) {
		...
		// take "hij" as a last resort
		if (proElement == null) proElement = lexicon.lookupWord("hij", LexicalCategory.PRONOUN);
...
//				modifierAdjPhrase.getFeatureAsElementList(InternalFeature.COMPLEMENTS);
			...
			// Copy the number and gender from the noun phrase to help with inflection later.
			Object nounPhraseNumber = nounPhrase.getFeature(Feature.NUMBER);
			Object nounPhraseGender = nounPhrase.getFeature(LexicalFeature.GENDER);
			Object specifier = nounPhrase.getFeatureAsString(InternalFeature.SPECIFIER);


			modifierElement.setFeature(InternalFeature.SPECIFIER, specifier);
			modifierElement.setFeature(Feature.NUMBER, nounPhraseNumber);
			modifierElement.setFeature(LexicalFeature.GENDER, nounPhraseGender);

			if (modifierHead.getFeatureAsBoolean(LexicalFeature.PREDICATIVE)) {
