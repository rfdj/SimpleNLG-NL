 ...
 */
package simplenlg.orthography.dutch;
 ...
 * <code>DocumentElement</code>s for Dutch.
 *  
		...
		for (int index = 0; index < length; index++) {
			...
			if ( DiscourseFunction.CONJUNCTION.equals(realisedChild
				...
				boolean repeated = realisedChild
					.getFeatureAsBoolean(DutchLexicalFeature.REPEATED_CONJUNCTION);
				if (index == 0) {
					if (repeated) {
					...
					if (!realisedChild.getFeatureAsBoolean(DutchLexicalFeature.NO_COMMA)) {
					realisation.append(", "); //$NON-NLS-1$
					}
					realisedChild = realisedChild.realiseOrthography();
					realisation.append(realisedChild.getRealisation());
			...
			if (childRealisation != null && !childRealisation.isEmpty() ) {
				...
				if (!separatorAdded && i < components.size() - 1) {
					...
					if (imperative
							...
							&& nextElement.getFeatureAsBoolean(DutchInternalFeature.CLITIC)) {
						if (lastChar != '\'') {
							realisation.append('-');
							dashAdded = true;
						}
					...
					if (nextElement.isA(PhraseCategory.CLAUSE)
							&& nextElement.getFeature(Feature.INTERROGATIVE_TYPE) == null
							&& !thisElement.isA(PhraseCategory.CANNED_TEXT)) {
						realisation.append(",");
						// If the previous clause ended in an apostrophe, do add whitespace
