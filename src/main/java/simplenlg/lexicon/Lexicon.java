...
public abstract class Lexicon {
	...
	public WordElement getAdditionCoordConjunction() {
		...
		if (hasWordByID("E0008890")) {
			...
			switch (getLanguage()) {
			case DUTCH:
				conjunction = lookupWord("en", LexicalCategory.CONJUNCTION);
				break;
			case FRENCH :
	...
	public WordElement getPassivePreposition() {
		...
		if (hasWordByID("E0014539")) {
			...
			switch (getLanguage()) {
				case DUTCH :
					preposition = lookupWord("door", LexicalCategory.PREPOSITION);
					break;
				case FRENCH :
	...
	public WordElement getDefaultComplementiser() {
		...
		switch (getLanguage()) {
		case DUTCH :
			complementiser = lookupWord("dat", LexicalCategory.COMPLEMENTISER);
			break;
		case FRENCH :
