...
//	private static final String WORD_REGEX = "\\w*";
	// modified by vaudrypl (Unicode letter categories)
	// modified by rfdj (include hyphens)
	private static final String WORD_REGEX = "(\\p{Lu}|\\p{Ll}|\\p{Lt}|\\p{Lm}|\\p{Lo}|-)*";

	/**
	 * Creates a new phrase factory with default associated lexicon.
	 */
	public NLGFactory() {
		this(null);
	}
	 ...
	 * Changed by vaudrypl :
	 * Passing null will result in the default lexicon being attributed.
	 * 
	 * @param newLexicon
	 ...
	 * <code>null</code> will not remove any existing lexicon from the factory,
	 * but will put the default lexicon instead.
	 * 
	 * @param newLexicon
	...
	public Language getLanguage() {
		return this.lexicon.getLanguage();
	}
	...
	public NLGElement createWord(Object word, LexicalCategory category) {
		...
		if (word instanceof NLGElement) {
			...
			// vaudrypl added condition on language
			if ( getLanguage() == Language.ENGLISH && PRONOUNS.contains(word)) {
...
//		setPhraseHead(phraseElement, prepositionalElement);
		phraseElement.setPreposition(preposition);

		if (complement != null) {
			// changed by vaudrypl
//			phraseElement.setObject(complement);
	...
	public NLGElement createNLGElement(Object element, LexicalCategory category) {
		...
		// InflectedWordElement - return underlying word
			...
			InflectedWordElement inflElement = (InflectedWordElement) element;
			NLGElement returnedWord = inflElement.getBaseWord();
//			for (String featureName : inflElement.getAllFeatureNames()) {
//				Object featureValue = inflElement.getFeature(featureName);
//				returnedWord.setFeature(featureName, featureValue);
//			}
			return returnedWord;
		}
	...
	public NPPhraseSpec createNounPhrase(Object specifier, Object noun) {
		if (noun instanceof NPPhraseSpec) {
			// changed by vaudrypl
			return new NPPhraseSpec((NPPhraseSpec)noun);
			//return (NPPhraseSpec) noun;
		}

		NPPhraseSpec phraseElement = new NPPhraseSpec(this);
		
		// Added by vaudrypl to check if the 'noun' is in fact a known pronoun
		// found in the lexicon; relevant only for String or StringElement.
		// It wasn't necessary in the English only version, because pronoun features
		// weren't found in the lexicon, but in setPronounFeatures(...)
		String nounString = null;
		if (noun instanceof String) nounString = (String) noun;
		else if (noun instanceof StringElement) {
			nounString = ((StringElement) noun).getRealisation();
		}
		NLGElement nounElement;
		if ( nounString != null && lexicon.hasWord(nounString, LexicalCategory.PRONOUN) ) {
			nounElement = createNLGElement(noun, LexicalCategory.PRONOUN);
		}
		else nounElement = createNLGElement(noun, LexicalCategory.NOUN);
	...
	public SPhraseSpec createClause(Object subject, Object verb,
		...
		// vaudrypl added conditional call to setVerbPhrase()
		if (verb instanceof VPPhraseSpec) {
			phraseElement.setVerbPhrase((VPPhraseSpec)verb);
		} else if (verb != null) {
			phraseElement.setVerb(verb);
		}
	 ...
	 * @return a <code>PhraseElement</code> representing this phrase.
	 */
	// return type changed from NLGElement by vaudrypl
	public StringElement createStringElement(String text) {
	...
	public CoordinatedPhraseElement createCoordinatedPhrase() {
		return new CoordinatedPhraseElement(this);
	}
	...
	public CoordinatedPhraseElement createCoordinatedPhrase(Object coord1,
			Object coord2) {
		return new CoordinatedPhraseElement(this, coord1, coord2);
	}
	...
	public DocumentElement createDocument(String title) {
		return new DocumentElement(DocumentCategory.DOCUMENT, title, this);
	}
	...
	public DocumentElement createDocument(String title,
		...
		DocumentElement document = new DocumentElement(
				DocumentCategory.DOCUMENT, title, this);
		if (components != null) {
	...
	public DocumentElement createList() {
		return new DocumentElement(DocumentCategory.LIST, null, this);
	}
	...
	public DocumentElement createList(List<DocumentElement> textComponents) {
		DocumentElement list = new DocumentElement(DocumentCategory.LIST, null, this);
	...
	public DocumentElement createListItem() {
		return new DocumentElement(DocumentCategory.LIST_ITEM, null, this);
	}
	...
	public DocumentElement createListItem(NLGElement component) {
		DocumentElement listItem = new DocumentElement(
				DocumentCategory.LIST_ITEM, null, this);
	...
	public DocumentElement createParagraph() {
		return new DocumentElement(DocumentCategory.PARAGRAPH, null, this);
	}
	...
	public DocumentElement createParagraph(List<DocumentElement> components) {
		DocumentElement paragraph = new DocumentElement(
				DocumentCategory.PARAGRAPH, null, this);
		if (components != null) {
	...
	public DocumentElement createSection() {
		return new DocumentElement(DocumentCategory.SECTION, null, this);
	}
	...
	public DocumentElement createSection(String title) {
		return new DocumentElement(DocumentCategory.SECTION, title, this);
	}
	...
	public DocumentElement createSentence() {
		return new DocumentElement(DocumentCategory.SENTENCE, null, this);
	}
	...
	public DocumentElement createSentence(List<NLGElement> components) {
		DocumentElement sentence = new DocumentElement(
				DocumentCategory.SENTENCE, null, this);
